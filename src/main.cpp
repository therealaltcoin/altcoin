...
uint256 hashGenesisBlock("0xc26c4f2e5b88c9e4308b5f469c6cbd3e20c411c44b7d8cce0a509d9fee9c3b2a");
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uATC
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            dFreeCount *= pow(1.0 - 1.0/512.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 262141 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 262141);
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    if (mapBlockIndex.count(hash) && hash != hashGenesisBlock)
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1391720763, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x761afffd << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x76, 0x1a, 0xff, 0xfd };
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running AltcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AltcoinMiner terminated\n");
        throw;
    }
}
