...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ATC unit input in lnReqAmount, we generate the URI with ATC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ATC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
